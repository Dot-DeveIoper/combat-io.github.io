...
(function (e) {
  ...
  function accessoriesMenu(player) {
    for (let i = 0; i < Acc.length; i++) {
      ...
      AccItems.innerHTML = `\n <div class="itemsInfo1">\n <img src="${
        ...
        Acc[i].price
      }</span>\n <a id="EquipAccessories${[
        i,
      ]}" class="equipItem">Equip</a>\n </div>\n `;
      Acc[i].clicked = false;
    ...
    if (soundOn) {
      if (AudioOn === "true") {
        ...
        const fadeInAudio = setInterval(() => {
          if (audio1.volume < 1) {
            audio1.volume += 0.011111111111111112;
          }
          if (audio1.volume > 1) {
  ...
  setInterval(() => {
    document.querySelector("#nameInput").value =
      localStorage.getItem("name") || "Player_0";
  }, 100);
  ...
  function update() {
    ...
    ctx.fillRect(
      0,
      mapSize - myPlayer.y + canvas.height / 2 - snowHeight - 3e3 - 1500,
      canvas.width,
      1500
    );
    ...
    function m(num, s) {
      ...
      return (num / f[i].value).toFixed(s).replace(y, "$1") + f[i].symbol;
    }
    if (leaderboard[0]) {
  ...
  function connect() {
    ws = new WebSocket("wss://kvb63w-8000.preview.csb.app.glitch.me/websocket");
